...
class MyResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.include_top:
            ...
            # x = self.fc(x)
        ret_dict = {}
        if 'rx' in self.label_choose:
            ret_dict['rx'] = self.rx(x)
        if 'ry' in self.label_choose:
            ret_dict['ry'] = self.ry(x)
        if 'rz' in self.label_choose:
            ret_dict['rz'] = self.rz(x)
        if 'tx' in self.label_choose:
            ret_dict['tx'] = self.tx(x)
        if 'ty' in self.label_choose:
            ret_dict['ty'] = self.ty(x)
        if 'tz' in self.label_choose:
            ret_dict['tz'] = self.tz(x)
        return ret_dict
