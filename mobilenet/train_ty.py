...
def parse_args():
    ...
    parser.add_argument('--pthpath', type=str, default='/content/drive/MyDrive/MyDRR/mobilenet/mobilenet_ty.pth')
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(len(label_choose)):
            ...
            if val_acc_avg[i] >= best_acc[i]:
                ...
                torch.save(net.state_dict(), os.path.join(save_root, 'mobilenet_ty_' + label_choose[i] + '.pth'))
        print(print_out+'\n'+f'lr: {scheduler.get_last_lr()}')
        result_lst.append(print_out)
        torch.save(net.state_dict(), os.path.join(save_root, 'mobilenet_ty_last.pth'))
    print('Finished Training')
    with open(os.path.join(save_root, 'result_ty.txt'), 'w') as f:
