...
if __name__ == '__main__':
    with open('imagelabel_new1.json', 'r') as f:
        label_dict = json.load(f)
    with open('errimg.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
            line = line.replace('\'', '')
            errimglst0 = line.split(', ')
    #删除列表errimglst中相同的元素
    errimglst = []
    for errimg in errimglst0:
        if errimg not in errimglst:
            errimglst.append(errimg)
    print(len(errimglst))
    #随机选择20张图片
    errimglst = random.sample(errimglst, 20)
    definejson = {}
    for img_name in errimglst:
        pre_label, topk2_label, pre_value, topk2_value = main(img_name)
        # print(pre_label)
        img_num = img_name.split('\\')[-1]
        img_num = img_num.split('.')[0]
        true_label = label_dict[img_num]
        x_pre = []
        value_pre = []
        value_topk2 = []
        topk2 = []
        ...
        for key in pre_label.keys():
            ...
            value_pre.append(float(pre_value[key]))
            value_topk2.append(float(topk2_value[key]))
            x_true.append(int(true_label[key]))
        definejson[img_num] = {
            'x_pre': x_pre,
            'value_pre': value_pre,
            'x_true': x_true,
            'topk2': topk2,
            'value_topk2': value_topk2
        }
    #保存
    with open('../photodraw/definejson.json', 'w') as f:
        json.dump(definejson, f)
    # with open('errimg.txt', 'r') as f:
    #     lines = f.readlines()
    #     for line in lines:
    #         line = line.replace('\'', '')
    #         errimglst = line.split(', ')
    # errimgjson = {}
    # for errimg in errimglst:
    #     img_num = errimg.split('\\')[-1]
    #     img_num = img_num.split('.')[0]
    #     errimgjson[img_num] = errimgjson.get(img_num, 0) + 1
    # with open('errimg1.json', 'w') as f:
    #     json.dump(errimgjson, f)
